...
export function approvedUser(user) {
  return user?.emailAddresses?.some(x => approvedEmails.has(x.emailAddress?.toLowerCase()) && x.verification?.status === 'verified') || false;
}
export async function authorize(req, res) {
  ...
  try {
    ...
    const email = user.emailAddresses.find(x => approvedEmails.has(x.emailAddress?.toLowerCase()) && x.verification?.status === 'verified').emailAddress.toLowerCase();
